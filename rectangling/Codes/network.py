...
#import utils.torch_tps_grid_gen as torch_tps_grid_gen
...
def build_model(net, input_tensor,mask_tesnor, iter_num):
    batch_size, _, img_h, img_w = input_tensor.size()


    flow_list = net(input_tensor, mask_tesnor, iter_num)
...
class CoupledTPS_RectanglingNet(nn.Module):

    def __init__(self):
        super(CoupledTPS_RectanglingNet, self).__init__()
        ...
        self.regressNet_part2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            #nn.Dropout(p=0.1, inplace=False),
            nn.Linear(in_features=1024, out_features=(grid_h+1)*(grid_w+1)*2, bias=True)
        )
    ...
    # forward
    def forward(self, input_tesnor, mask_tesnor, iter_num):
        batch_size, _, img_h, img_w = input_tesnor.size()

        feature_ori = self.feature_extractor(torch.cat([input_tesnor, mask_tesnor], 1))
